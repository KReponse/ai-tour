...
export const getAllToursAdmin = async (req, res) => {
  try {
    ...
    const listings = await Listing.find(filter) // ✅ Changed from Tour
      .populate("provider", "name email")
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit);

    const total = await Listing.countDocuments(filter); // ✅ Changed from Tour

    res.status(200).json({
      success: true,
      count: listings.length,
      tours: listings, // Keep field name for backward compatibility
      total,
      page: parseInt(page),
      limit: parseInt(limit),
      totalPages: Math.ceil(total / limit)
...
export const approveTourAdmin = async (req, res) => {
  try {
    const listing = await Listing.findById(req.params.id); // ✅ Changed from Tour
    ...
    await createNotification({
      ...
      sender: req.user._id,
      type: 'listing_approved', // ✅ Changed from 'tour_approved'
      title: 'Tour Approved ✅',
      message: `Your tour "${listing.title}" has been approved and is now visible to travelers.`,
      data: { listingId: listing._id }, // ✅ Changed from tourId
      link: `/provider/listings/${listing._id}` // ✅ Changed from tours
    });
...
export const rejectTourAdmin = async (req, res) => {
  try {
    ...
    await createNotification({
      recipient: listing.provider,
      sender: req.user._id,
      type: 'listing_rejected', // ✅ Changed from 'tour_rejected'
      title: 'Tour Rejected ❌',
  ...
  try {
    ...
    recentBookings.forEach(booking => {
      activities.push({
        id: booking._id,
        type: 'booking',
        title: `New Booking: ${booking.listing?.title || 'Experience'}`, // ✅ Changed from tour
