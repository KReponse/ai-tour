...
const ProviderNavbar = ({
  ...
  const userId = user?._id; // ✅ Stable user ID for dependencies
  ...
  }, [userId]); // ✅ Only depends on user ID, not the whole user object

  // ✅ Fetch notifications only once on mount and when user changes
  useEffect(() => {
    if (user && !hasFetchedRef.current) {
      fetchNotifications();
      hasFetchedRef.current = true;
    }
  }, [userId, fetchNotifications]); // ✅ Depends on user ID only
  ...
  }, [notificationOpen, userId, fetchNotifications]); // ✅ Depends on user ID only
  ...
  }, [userId, refreshUser]); // ✅ Depends on user ID only
